...
class ApodGraphQlIntegrationTest {
    ...
    void noSelectorReturnsTodayAsOnePicture() {
        ...
        ExecutionResult result = queryExecutor.execute("{ pictures { date title relevanceScore } }");

        assertThat(result.getErrors()).isEmpty();
        assertThat(result.toSpecification().toString())
                .contains("pictures=[{date=2026-09-04, title=A title, relevanceScore=null}]");
    ...
    void dateSelectorReturnsOnePictureAndSimilarUsesItsDate() {
        ...
        assertThat(result.getErrors()).isEmpty();
        assertThat(result.toSpecification().toString())
                .contains("pictures=[{date=2019-04-11", "similar=[{date=2019-04-12, title=A title, relevanceScore=0.8}]");
    ...
    void rejectsDateAndSearchTogether() {
        ExecutionResult result = queryExecutor.execute("""
                { pictures(date: "2019-04-11", search: "germany") { title } }
                """);
    ...
    void similarityFailureIsIsolatedToTheNestedField() {
        ...
        ExecutionResult result = queryExecutor.execute("{ pictures { title similar(limit: 5) { title } } }");

        assertThat(result.getErrors()).singleElement().satisfies(error -> {
            assertThat(error.getPath()).containsExactly("pictures", 0, "similar");
            assertThat(error.getExtensions()).containsEntry("code", "SIMILARITY_UNAVAILABLE");
        });
        Map<String, Object> data = result.getData();
        assertThat((List<?>) data.get("pictures")).singleElement().asInstanceOf(org.assertj.core.api.InstanceOfAssertFactories.MAP)
